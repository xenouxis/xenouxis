<?php
    ...
    $req = $bdd->prepare("SELECT * FROM peau WHERE id=?");
    $req->execute([$id]);
    $don = $req->fetch();
    if(!$don)
    {
    ...
    if(isset($_POST['nom']))
    {
        ...
        if($error==0)
        {
            if(empty($_FILES['fichier']['tmp_name']))
            {
                ...
                $update = $bdd->prepare("UPDATE peau SET nom=?, description=?, date=? WHERE id=?");
                $update->execute([$nom, $description, $date, $id]);
                $update->closeCursor();
                header("LOCATION:peau.php?update=".$id);
            }else{
                ...
                if($error == 0)
                {
                    ...
                    if(move_uploaded_file($_FILES['fichier']['tmp_name'], $dossier.$fichiercptl))
                    {
                        ...
                        $update = $bdd->prepare("UPDATE peau SET nom=?, description=?, date=?, fichier=? WHERE id=?");
                        $update->execute([$nom, $description, $date, $fichiercptl,$id]);
                        $update->closeCursor();
                        header("LOCATION:peau.php?update=".$id);
                    }else{
                        header("LOCATION:addPeau.php?errorimg=4");
                    }

                }else{
                    header("LOCATION:updatePeau.php?id=".$id."&errorimg=".$error);
                }     
            }
        }else{
            // redirection vers le formulaire en indiquant l'erreur
            header("LOCATION:updatePeau.php?id=".$id."&error=".$error);
        }
