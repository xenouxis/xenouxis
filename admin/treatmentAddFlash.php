        ...
        if($error==0)
        {
            ...
            if($error == 0)
            {
                ...
                if(move_uploaded_file($_FILES['fichier']['tmp_name'], $dossier.$fichiercptl))
                {
                    ...
                    require "../connexion.php";
                    $insert = $bdd->prepare("INSERT INTO flash(nom,fichier,description,date) VALUES(?,?,?,?)");
                    $insert->execute([$nom,$fichiercptl,$description,$date]);
                    $insert->closeCursor();
                    header("LOCATION:flash.php?add=success");
                }else{
                    header("LOCATION:addFlash.php?errorimg=4");
                }

            }else{
                header("LOCATION:addFlash.php?errorimg=".$error);
            }
            ...
            header("LOCATION:addFlash.php?error=".$error);
        }
