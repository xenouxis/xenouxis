        ...
        if($error==0)
        {
            ...
            if($error == 0)
            {
                ...
                if(move_uploaded_file($_FILES['fichier']['tmp_name'], $dossier.$fichiercptl))
                {
                    ...
                    require "../connexion.php";
                    $insert = $bdd->prepare("INSERT INTO peau(nom,fichier,description,date) VALUES(?,?,?,?)");
                    $insert->execute([$nom,$fichiercptl,$description,$date]);
                    $insert->closeCursor();
                    header("LOCATION:peau.php?add=success");
                }else{
                    header("LOCATION:addPeau.php?errorimg=4");
                }

            }else{
                header("LOCATION:addPeau.php?errorimg=".$error);
            }
            ...
            header("LOCATION:addPeau.php?error=".$error);
        }
