<?php 
    ...
    if(isset($_GET['delete']))
    {
        ...
        $reqdel = $bdd->prepare("SELECT * FROM photoshop  WHERE id=?");
        ...
        $delete = $bdd->prepare("DELETE FROM photoshop WHERE id=?");
        $delete->execute([$idProd]);
        $delete->closeCursor();
        header("LOCATION:photoshop.php?delsuccess=".$idProd);
...
<body>
    ...
    <div class="container-fluid">
     <h2>Gestion des créations photoshop</h2>
     <a href="addPhotoshop.php" class="btn btn-primary">Ajouter une création photoshop</a>
     <?php 
        if(isset($_GET['add']))
        {
    ...
    ?>
     <table class="table table-striped">
        ...
        <tbody>
            <?php 
                $req = $bdd->query("SELECT * FROM photoshop");
                while($don = $req->fetch())
                {
                    echo "<tr>";
                        echo "<td>".$don['id']."</td>";
                        ...
                        echo "<td>".$don['date']."</td>";
                        echo "<td>";
                            echo "<a href='updatePhotoshop.php?id=".$don['id']."' class='btn btn-warning'>Modifier</a>";
                            echo "<a href='photoshop.php?delete=".$don['id']."' class='btn btn-danger'>Supprimer</a>";
                        echo "</td>";
                    echo "</tr>";
                }
                $req->closeCursor();
