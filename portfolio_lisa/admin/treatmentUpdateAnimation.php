    ...
    if(isset($_POST['nom']))
    {
        ...
        if($error==0)
        {
            if(empty($_FILES['fichier']['tmp_name']))
            {
                ...
                $update = $bdd->prepare("UPDATE products SET nom=?, categorie=?, description=?, date=?, WHERE id=?");
                ...
                if($error == 0)
                {
                    ...
                    if(move_uploaded_file($_FILES['fichier']['tmp_name'], $dossier.$fichiercptl))
                    {
                        ...
                        $update = $bdd->prepare("UPDATE infographie SET nom=?, description=?, date=?, fichier=? WHERE id=?");
                        $update->execute([$nom, $description, $date,$fichiercptl,$id]);
                        $update->closeCursor();
                        header("LOCATION:animation.php?update=".$id);
                    }else{
                        header("LOCATION:addAnimation.php?errorimg=4");
                    }

                }else{
                    header("LOCATION:updateAnimation.php?id=".$id."&errorimg=".$error);
                }     
            }
        }else{
            // redirection vers le formulaire en indiquant l'erreur
            header("LOCATION:updateAnimation.php?id=".$id."&error=".$error);
        }
