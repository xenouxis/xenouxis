        ...
        if($error==0)
        {
            ...
            if($error == 0)
            {
                ...
                if(move_uploaded_file($_FILES['fichier']['tmp_name'], $dossier.$fichiercptl))
                {
                    ...
                    require "../connexion.php";
                    $insert = $bdd->prepare("INSERT INTO photoshop(nom,fichier,description,date) VALUES(?,?,?,?)");
                    $insert->execute([$nom,$fichiercptl,$description,$date]);
                    $insert->closeCursor();
                    header("LOCATION:photoshop.php?add=success");
                }else{
                    header("LOCATION:addPhotoshop.php?errorimg=4");
                }

            }else{
                header("LOCATION:addPhotoshop.php?errorimg=".$error);
            }
            ...
            header("LOCATION:addPhotoshop.php?error=".$error);
        }
