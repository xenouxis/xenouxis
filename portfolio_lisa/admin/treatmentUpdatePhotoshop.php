<?php
    ...
    $req = $bdd->prepare("SELECT * FROM photoshop WHERE id=?");
    $req->execute([$id]);
    $don = $req->fetch();
    if(!$don)
    {
    ...
    if(isset($_POST['nom']))
    {
        ...
        if($error==0)
        {
            if(empty($_FILES['fichier']['tmp_name']))
            {
                ...
                $update = $bdd->prepare("UPDATE photoshop SET nom=?, description=?, date=? WHERE id=?");
                $update->execute([$nom, $description, $date, $id]);
                $update->closeCursor();
                header("LOCATION:photoshop.php?update=".$id);
            }else{
                ...
                if($error == 0)
                {
                    ...
                    if(move_uploaded_file($_FILES['fichier']['tmp_name'], $dossier.$fichiercptl))
                    {
                        ...
                        $update = $bdd->prepare("UPDATE photoshop SET nom=?, description=?, date=?, fichier=? WHERE id=?");
                        $update->execute([$nom, $description, $date, $fichiercptl,$id]);
                        $update->closeCursor();
                        header("LOCATION:photoshop.php?update=".$id);
                    }else{
                        header("LOCATION:addPhotoshop.php?errorimg=4");
                    }

                }else{
                    header("LOCATION:updatePhotoshop.php?id=".$id."&errorimg=".$error);
                }     
            }
        }else{
            // redirection vers le formulaire en indiquant l'erreur
            header("LOCATION:updatePhotoshop.php?id=".$id."&error=".$error);
        }
