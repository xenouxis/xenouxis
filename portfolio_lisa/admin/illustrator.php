<?php 
    ...
    if(isset($_GET['delete']))
    {
        ...
        $reqdel = $bdd->prepare("SELECT * FROM illustrator WHERE id=?");
        ...
        if(!$dondel = $reqdel->fetch())
        {
            ...
            header("LOCATION:illustrator.php");
        }
        ...
        $delete = $bdd->prepare("DELETE FROM illustrator WHERE id=?");
        $delete->execute([$idProd]);
        $delete->closeCursor();
        header("LOCATION:illustrator.php?delsuccess=".$idProd);
...
<body>
    ...
    <div class="container-fluid">
     <h2>Gestion des créations illustrator</h2>
     <a href="addIllustrator.php" class="btn btn-primary">Ajouter une création illustrator</a>
     <?php 
        if(isset($_GET['add']))
        {
    ...
    ?>
     <table class="table table-striped">
        ...
        <tbody>
            <?php 
                $req = $bdd->query("SELECT * FROM illustrator");
                while($don = $req->fetch())
                {
                    echo "<tr>";
                        echo "<td>".$don['id']."</td>";
                        ...
                        echo "<td>".$don['date']."</td>";
                        echo "<td>";
                            echo "<a href='updateIllustrator.php?id=".$don['id']."' class='btn btn-warning'>Modifier</a>";
                            echo "<a href='illustrator.php?delete=".$don['id']."' class='btn btn-danger'>Supprimer</a>";
                        echo "</td>";
                    echo "</tr>";
                }
                $req->closeCursor();
