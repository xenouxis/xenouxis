        ...
        if($error==0)
        {
            ...
            if($error == 0)
            {
                ...
                if(move_uploaded_file($_FILES['fichier']['tmp_name'], $dossier.$fichiercptl))
                {
                    ...
                    require "../connexion.php";
                    $insert = $bdd->prepare("INSERT INTO infographie(nom,fichier,description,video,date) VALUES(?,?,?,?,?)");
                    $insert->execute([$nom,$fichiercptl,$description,$video,$date]);
                    $insert->closeCursor();
                ...
                header("LOCATION:addAnimation.php?errorimg=".$error);
            }
            ...
            header("LOCATION:addAnimation.php?error=".$error);
        }
