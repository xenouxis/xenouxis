    ...
    if(isset($_POST['nom']))
    {
        ...
        if($error==0)
        {
            if(empty($_FILES['fichier']['tmp_name']))
            {
                ...
                $update = $bdd->prepare("UPDATE web SET nom=?, description=?, site=?, date=? WHERE id=?");
                $update->execute([$nom, $description, $site, $date, $id]);
                $update->closeCursor();
                ...
                if($error == 0)
                {
                    ...
                    if(move_uploaded_file($_FILES['fichier']['tmp_name'], $dossier.$fichiercptl))
                    {
                        ...
                        $update = $bdd->prepare("UPDATE web SET nom=?, description=?, date=?, fichier=? WHERE id=?");
                        $update->execute([$nom, $description, $date, $fichiercptl, $site, $id]);
                        $update->closeCursor();
                        header("LOCATION:web.php?update=".$id);
                    }else{
